  ...
  <view class="container">
    ...
    <uni-forms ref="form" :rules="rules" :model="noteItem">
      ...
      <view v-if="funcode==1" class="optBox">
        <image class="status-image" :src="imgPath + img_status[noteItem.status]" mode=""></image>
        <uni-data-select class="selecter" v-model="value" :localdata="range" @change="change"></uni-data-select>
      </view>
...
export default {
  ...
  methods: {
    async setTheStorage(theType, hintMessage) {
      const phone = uni.getStorageSync('userPhone'); // 从本地存储中获取手机号
      if (!phone) {
        uni.showToast({
          title: '未找到手机号，请重新登录',
          icon: 'none'
    ...
    getTheItemFromCloud() {
      uniCloud.callFunction({
        ...
        this.noteItem.id = notes[0]._id;  // 返回只有一条记录的集合
